...
public class UploadServiceImpl implements UploadService {
  ...
  public boolean addUpload(MultipartHttpServletRequest multipartRequest) throws Exception {
    ...
    // upload.uploadNo가 userMapper의 실행으로 번호가 채워진다.(selectKey로 인해)
  ...
  public ResponseEntity<Resource> download(HttpServletRequest request) {
    ...
    // 사용자가 다운로드 받을 파일의 이름을 결정(User-Agent값에 따른 인코딩 처리)
  ...
  public ResponseEntity<Resource> downloadAll(HttpServletRequest request) {
    ...
    try {
      ...
      for(AttachDto attach : attachList) {
        ...
        // 각 첨부 파일의 원래 이름으로 zip 파일에 등록하기 (이름만 등록)
