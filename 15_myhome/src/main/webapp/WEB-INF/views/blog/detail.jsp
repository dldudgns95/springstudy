...
<div>
  ...
  <div>
    <form id="frm_comment_add">
      <textarea rows="3" cols="50" name="contents" id="contents" placeholder="댓글을 작성해주세요."></textarea>
      <input type="hidden" name="userNo" value="${sessionScope.user.userNo}">
      <input type="hidden" name="blogNo" value="${blog.blogNo}">
      <button type="button" id="btn_comment_add">댓글작성완료</button>
    </form>
  </div>
    ...
    const fnCommentList = () => {
  	  $.ajax({
  	    ...
  	    success: (resData) => {	// resData = {"commentList": [], "paging": "<div>...</div>"}
  		  ...
  		  $.each(resData.commentList, (i, c) => {
  		    ...
  		    if(c.status === 0) {
  		      ...
  		      str += '  <div>' + c.userDto.name + '</div>';
  	  		  ...
  	  		  str += '      <textarea rows="3" cols="50" class="reply_contents" name="contents" placeholder="답글을 입력하세요."></textarea>';
  	  		  ...
  	  		  if('${sessionScope.user.userNo}' == c.userDto.userNo) {
  		  		  ...
  		  		  str += '    <i class="fa-regular fa-circle-xmark icon_remove_comment"></i>';
  		  		  str += '  </div>';
  	  		  }
  	  		  str += '</div>';
  		    } 
  		  ...
  		  $('#paging').html(resData.paging);  // fnAjaxPaging() 함수가 호출되는 곳
  	    }
  	  })
    }
    ...
    const fnBlind = () => {
      $(document).on('click', '.btn_open_reply', (ev) => {
    	...
    	if(blindTarget.hasClass('blind')) {
    	  ...
    	  blindTarget.addClass('blind');              // 답글 입력화면 닫기
    	}
      })
    }
    ...
    const fnCommentReplyAdd = () => {
      $(document).on('click', '.btn_add_reply', (ev) => {
    	...
    	$.ajax({
    	  ...
    	  data: frmAddReply.serialize(),  // $(ev.target).parent().serialize() 과 같음
    	  // 응답
    	  dataType: 'json',
