...
CREATE TABLE ACCESS_T(
    ...
    CONSTRAINT FK_USER_ACCESS FOREIGN KEY(EMAIL) REFERENCES USER_T(EMAIL) ON DELETE CASCADE  -- SET NULL(USER_T의 EMAIL이 지워지면 NULL 처리)
);
...
-- 2. 가입할 때 이메일 중복 체크 (전부 NULL일때만 가입 가능)
SELECT EMAIL
  FROM USER_T
