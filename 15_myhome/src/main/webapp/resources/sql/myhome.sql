...
CREATE TABLE BLOG_T(
    ...
    USER_NO     NUMBER             NOT NULL,  -- 작성자 번호(NULL인 경우 ON DELETE SET NULL 처리가 가능하고, NOT NULL 인 경우 ON DELETE CASCADE 처리가 가능하다.)
    HIT         NUMBER             DEFAULT 0, -- 조회수
    IP          VARCHAR2(30 BYTE)  NULL,      -- IP 주소 (INSERT X)
    CREATED_AT  VARCHAR2(30 BYTE)  NULL,      -- 작성일자
    MODIFIED_AT VARCHAR2(30 BYTE)  NULL,      -- 수정일자
...
SELECT B.BLOG_NO, B.TITLE, B.CONTENTS, B.HIT, B.IP, B.CREATED_AT, B.MODIFIED_AT, U.USER_NO, U.EMAIL, U.NAME, I.IMAGE_PATH, I.FILESYSTEM_NAME
  FROM USER_T U INNER JOIN BLOG_T B
    ON U.USER_NO = B.USER_NO
                INNER JOIN BLOG_IMAGE_T I
    ON B.BLOG_NO = I.BLOG_NO
   AND B.BLOG_NO = 9;
...
-- FREE_SEQ.CURRVAL : FREE_SEQ의 현재 VALUE
...
INSERT INTO FREE_T VALUES(FREE_SEQ.NEXTVAL, 'user3@naver.com', '내용17', SYSTIMESTAMP, 1, 0, FREE_SEQ.CURRVAL, 0);
...
INSERT INTO FREE_T VALUES(FREE_SEQ.NEXTVAL, 'user1@naver.com', '내용22', SYSTIMESTAMP, 1, 0, FREE_SEQ.CURRVAL, 0);
COMMIT;
...
-- 3) 블로그 댓글 목록
-- 블로그 - 댓글 : 1:N (댓글이 달린 블로그의 정보는 이미 상세보기에 모두 표시되어 있으므로 여기에선 조인할 필요가 없다.)
